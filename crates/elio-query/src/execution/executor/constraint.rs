...
use elio_common::catalog::{ConstraintCatalogEntry, ConstraintKind};
use elio_common::mapb::IndexKeyCodec;
use elio_common::{LabelId, TokenKind};
...
pub struct LabelConstraints {
    ...
    pub constraints: Vec<(LabelId, ConstraintCatalogEntry)>,
}
    ...
    for (label_id, constraint) in &label_constraints.constraints {
        ...
        for row_idx in 0..props.len() {
            let extraction_result =
                extract_property_values_with_null_check(qctx, props, &constraint.property_key_ids, row_idx);
            ...
            match extraction_result {
                PropertyExtractionResult::Success(prop_values) => {
                    ...
                    if qctx.graph_store().unique_index_exists(
                        qctx.txn().as_ref(),
                        *label_id,
                        &constraint.property_key_ids,
                        &prop_value_refs,
                    )? {
    ...
    for (label_id, constraint) in &label_constraints.constraints {
        for (row_idx, node_opt) in nodes.iter().enumerate() {
            if let Some(node) = node_opt {
                let prop_values = extract_property_values(qctx, props, &constraint.property_key_ids, row_idx);
...
fn extract_property_values_with_null_check(
    ...
    for prop_key_id in prop_key_ids {
        let prop_name = match qctx.token_store().get_token_val(*prop_key_id, TokenKind::PropertyKey) {
            Ok(name) => name,
...
fn extract_property_values(
    ...
    match extract_property_values_with_null_check(qctx, props, prop_key_ids, row_idx) {
